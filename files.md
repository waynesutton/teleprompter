...
- `README.md` - Project overview, feature list, workflow, stack, Convex component notes, local setup, Auth/BYOK setup including HeyGen / HyperFrames, per-user library notes, and static hosting deploy commands.
...
- `convex/videoJobs.ts` - Convex query and mutation for authenticated per-user HyperFrames video job listing and creation, requiring one AI provider key, HeyGen / HyperFrames BYOK, and Firecrawl BYOK for URL jobs.
...
- `convex/userApiKeys.ts` - Convex queries and mutations for per-user encrypted AI, scraping, voice, and video rendering API key status, storage, and removal.
- `convex/apiKeyActions.ts` - Node Convex action that encrypts AI, scraping, voice, and video rendering user API keys before storing them.
- `convex/users.ts` - Convex query for the currently signed-in GitHub user profile and protected account deletion mutation for user-owned app/auth data, including video jobs.
...
- `prds/video-generation-tab.md` - Product requirements and completion log for the dedicated Video tab, HyperFrames job queue, and per-user video job storage.
- `prds/heygen-byok-video-gating.md` - Product requirements and completion log for making HeyGen / HyperFrames a per-user BYOK requirement for Video jobs.
